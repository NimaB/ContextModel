  ...
  
{
  ...
  if(argc < 5)
    {
      cerr<<"The format to run this file is:"<<endl;
      cerr<<"PreProc {pointcloud type} {input pointcloud path} {transformed pointcloud path}"
	  <<"{path to save cloud normals} {Path of Test Full Points pointcloud (TFP_cloud.pcd)}"
	  <<"{path to the copy of pointcloud in XYZRGBL type(optional)"<<endl;
      return 0;
    }
  else
    {
      ...
      if (atoi(argv[1]) == 1)
	{
	  ...
	  cout<<"Transferred PointCloud is saved."<<endl;
	  //--------------------------------------------------
	  cout<<"Estimating PointCloud Normals..."<<endl;
	...
	if (atoi(argv[1]) == 2)//This is when the input is of type RGBL
      ...
      if (argc > 5 )//This is when an address is provided for TestCloud to be saved in.
	{
	  cout<<"Generating FullTestPointCloud..."<<endl;
	...
	//And a path for RGBL version is provided.
	{
	...
	PCL_ERROR("Cloud2 save fail!");
      ...
      //----------------------------------------------------
      cout<<"Done with this PointCloud!"<<endl;
    }
}
