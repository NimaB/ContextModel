...
void FeatureExtract(pcl::PointCloud<pcl::PointXYZRGBL>::Ptr blob_in, pcl::PointXYZRGBL opoint, VectorXf * fvector)
{
  ...
  for(int i = 0; i < n_points; ++i)
    AvHeight = AvHeight + blob_in->points[i].y;
...
int main(int argc, char **argv)
{
  ...
  float B_Radius = 0.01;//radius of the querypoint's blob
  ...
  //------
    //------
    //Test
  //here we are tring to focus the pointcloud to the area around the object
    for (size_t i =0; i < Cloud_labeled->points.size() ; i++)
    {
  	  if(Cloud_labeled->points[i].label == 1)
  	  {
  		  cerr<<"The center selected index is : "<< i <<endl;
  		  BlobExtract(Cloud_labeled->points[i],Cloud_labeled,0.70,Cloud_labeled);
  		  break;
  	  }
    }
	  ...
	  if ( Cloud_Blob->size() >= BPnTresh)
	      ...
	      for(size_t j = 0; j < Search_Cloud->points.size(); ++j)
		{
		  ...
		  if ((i ==0) && (j == 0))//just for the first iteration
		    ...
		    OverWrite = 0;
		  WtoFile(CofSample,FVector,OverWrite,argv[2],argv[3],DaFiFormat);
